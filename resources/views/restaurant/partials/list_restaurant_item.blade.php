<div class="col-md-6">
    <div class="media" {!! $link?'onclick="window.location=\''. route('restaurant.view', ["type"=>$type, "id"=>$restaurant->id, "name"=>$restaurant->name]).'\'"':'' !!}>
        <div class="media-left">
            <a href="#">
                <img class="media-object" width="100px" height="100px" src="//foodadmin.local/images/restaurant/logo/{{ $restaurant->logo }}" alt="{{ $restaurant->name }}">
            </a>
        </div>
        <div class="media-body">
            <div class="media-heading">
                <h4>{{ strlen($restaurant->name)>25?substr($restaurant->name,0,24).'...':$restaurant->name }}</h4>
                <span class="help-block">{{ implode(', ', json_decode($restaurant->cuisines, true)) }}</span>
            </div>
